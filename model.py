...
class LeNetConvPoolLayer2(object):
	...
	def __call__(self, input):
		...
		conv_out = T.nnet.conv2d(input=input, filters=self.W, filter_shape=self.filter_shape, input_shape=self.image_shape) # new code
	...
	def predict(self, new_data, batch_size):
		...
		conv_out = T.nnet.conv2d(input=new_data, filters=self.W, filter_shape=self.filter_shape, input_shape=img_shape) # new code
		if self.non_linear=="tanh":
			conv_out_tanh = T.tanh(conv_out + self.b.dimshuffle('x', 0, 'x', 'x'))
